...
async function connectToWhatsApp() {
  if (isConnecting) {
    console.log('⏳ Ya está intentando conectar...');
    return;
  }
  
  isConnecting = true;
  console.log('🔌 [CONEXIÓN] Iniciando conexión WhatsApp...');
  ...
  try {
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'close') {
        ...
        console.log(`❌ Desconectado. Razón: ${reason}. Reconectando: ${shouldReconnect}`);
        
        if (shouldReconnect) {
          console.log('🔄 Esperando 5 segundos antes de reconectar...');
          setTimeout(() => connectToWhatsApp(), 5000);
        } else {
          console.log('⚠️ Usuario deslogueado. Necesita escanear QR nuevamente.');
        }
      }
    });
    ...
    console.error('❌ Error connectToWhatsApp:', error.message);
    isConnecting = false;
...
async function handleConfirm(text, from) {
  ...
  try {
    ...
    console.log('🟢 Orden encontrada:', order.easy_cod_order_id);
